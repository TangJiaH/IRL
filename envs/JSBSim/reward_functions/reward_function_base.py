...
class BaseRewardFunction(ABC):
    ...
    def _process(self, new_reward, agent_id, render_items=()):
        ...
        # print(f"_process--agent_id:{agent_id}-is_potential:{self.is_potential}-pre_reward:{self.pre_rewards[agent_id]}")
        if self.is_potential:
            reward, self.pre_rewards[agent_id] = reward - self.pre_rewards[agent_id], reward
        # print(f"new_reward:{new_reward}-final_reward:{reward}")
