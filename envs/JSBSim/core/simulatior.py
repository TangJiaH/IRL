...
class AircraftSimulator(BaseSimulator):
    ...
    def __init__(self,
        ...
        self.dead_reward_flag = False  # 控制我方被击中或坠机后的奖励只叠加一次
    ...
    @property
    def left_missiles(self):
        return self.num_left_missiles
...
class MissileSimulator(BaseSimulator):
    ...
    def __init__(self,
        ...
        self.hit_reward_flag = False  # 击中奖励位，未击中或已奖励为False
        self.miss_reward_flag = 0  # 被规避奖励位，未击中或已规避为False
    ...
    def get_parent(self):
        return self.parent_aircraft

    def get_target(self):
        return self.target_aircraft
    ...
    def run(self):
        ...
        if self.is_alive:
            if distance < self._Rc and self.target_aircraft.is_alive:
            ...
            elif (self._t > self._t_max) or ((np.linalg.norm(self.get_velocity()) < self._v_min) and (self._t > 2)) \
                    or np.sum(
                ...
                if self.miss_reward_flag == 0:
                    self.miss_reward_flag = 1
            else:
    ...
    def hit_reward(self):
        self.hit_reward_flag = False

    def miss_reward(self):
        self.miss_reward_flag = 2

    def is_hit(self):
        return self.hit_reward_flag

    def is_miss(self):
        return self.miss_reward_flag == 1
