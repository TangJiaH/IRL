...
def _get_ppo_config(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--bc-regularization", action='store_true', default=False,
                       help="Enable BC policy regularization during PPO fine-tuning.")
    group.add_argument("--bc-model-dir", type=str, default=None,
                       help="Path to BC model directory containing actor_latest.pt.")
    group.add_argument("--bc-coef", type=float, default=0.1,
                       help="BC regularization coefficient.")
    return parser
...
def _get_selfplay_config(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--use-adversarial", action='store_true', default=False,
                       help="By default false. If set, use selfplay algorithms.")
...
def _get_render_config(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--render-opponent-index", type=str, default=0,
                       help="the index of opponent policy in the opponent pool. by default latest")
    group.add_argument("--render-index", type=str, default=0, help="the index of ego policy. by default latest")
    return parser
