...
class SelfplayJSBSimRunner(JSBSimRunner):
    ...
    def insert(self, data: List[np.ndarray]):
        ...
        self.opponent_rnn_states[dones_env == True] = np.zeros(((dones_env == True).sum(), *rnn_states_actor.shape[1:]),
                                                               dtype=np.float32)
    ...
    def eval(self, total_num_steps):
        ...
        eval_infos['opponent_eval_average_episode_rewards'] = opponent_average_episode_rewards.mean()
        ...
        logging.info(
            "opponent eval average episode rewards: " + str(eval_infos['opponent_eval_average_episode_rewards']))
        logging.info(" latest elo score: " + str(self.latest_elo))
        logging.info(" opponent elo score: " + str(opponent_elo))
    ...
    def save(self, episode):
        ...
        elo_history = {
            ...
            "current_episode": episode  # 保存当前训练轮次
        }
        with open(os.path.join(self.save_dir, 'elo_history.json'), 'w') as f:
    ...
    def render(self):
        ...
        # logging.info(f"opponent_idx:{opponent_idx}")
        ...
        print(f'{file_path}/{self.experiment_name}.txt.acmi')
