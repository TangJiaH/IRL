...
class BaseTask(ABC):
    ...
    def get_reward(self, env, agent_id, info={}) -> Tuple[float, dict]:
        ...
        reward = 0.0
        for reward_function in self.reward_functions:
            reward_item = reward_function.get_reward(self, env, agent_id)
            reward += reward_item
        # print(f"agent_id:{agent_id},base_reward:{reward}")
        return reward, info

    # 飞机坠机后也应该继续计算导弹命中奖励（用于计算一些飞机坠毁后的奖励）
    def get_die_reward(self, env, agent_id, info={}) -> Tuple[float, dict]:
        """
        Aggregate reward functions
        ...
        """
        reward = 0.0
        for reward_function in self.die_reward_functions:
            reward_item = reward_function.get_reward(self, env, agent_id)
            reward += reward_item
        # print(f"agent_id:{agent_id},base_reward:{reward}")
        return reward, info
