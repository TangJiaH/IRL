...
from ..my_reward.boundary_reward import BoundaryReward
from ..my_reward.control_reward import ControlReward
from ..my_reward.dead_reward import DeadReward
from ..my_reward.lock_reward import LockReward
from ..my_reward.process_reward import ProcessReward
from ..my_reward.result_reward import ResultReward
from ..termination_conditions import ExtremeState, LowAltitude, Overload, Timeout, SafeReturn, OutBoundary
...
class SingleCombatTask(BaseTask):
    def __init__(self, config):
        ...
        self.reward_functions = [
            BoundaryReward(self.config),
            ProcessReward(self.config),
            ResultReward(self.config),
            LockReward(self.config)
        ]

        self.die_reward_functions = [
            DeadReward(self.config)
        ]
    ...
    def get_obs(self, env, agent_id):
        ...
        - relative enm info
            ...
            - [11] ego_AO(attack_angle)                (unit: rad) [0, pi]
        ...
        norm_obs[9] = (enm_obs_list[9] - ego_obs_list[9]) / 340  # 高度差
        norm_obs[10] = (enm_obs_list[2] - ego_obs_list[2]) / 1000  # 速度差
        norm_obs[11] = ego_AO
        norm_obs[12] = ego_TA
        norm_obs[13] = R / 10000
    ...
    def get_reward(self, env, agent_id, info=...):
        if self._agent_die_flag.get(agent_id, False):
            return super().get_die_reward(env, agent_id, info=info)
        else:
            self._agent_die_flag[agent_id] = not (env.agents[agent_id].is_alive or env.agents[agent_id].is_lockon)
...
class HierarchicalSingleCombatLockTask(SingleCombatTask):
    ...
    def __init__(self, config: str):
        ...
        self.min_lock_step = getattr(self.config, 'min_lock_step', 125)
        self.min_shoot_step = getattr(self.config, 'min_shoot_step', 125)
    ...
    def reset(self, env):
        ...
        self.shoot_flag = {agent_id: 2000 for agent_id in env.agents.keys()}
        self._lock_time = {agent_id: 0 for agent_id in env.agents.keys()}
        return super().reset(env)

    def step(self, env):
        SingleCombatTask.step(self, env)
        for agent_id, agent in env.agents.items():
            # print(f"agent_id:{agent_id}----status:{agent.is_alive or agent.is_lockon}")
            if not (agent.is_alive or agent.is_lockon):
                continue
            ego_feature = np.hstack([env.agents[agent_id].get_position(),
                                     env.agents[agent_id].get_velocity()])
            for enm in env.agents[agent_id].enemies:
                if enm.is_alive or enm.is_lockon:
                    enm_feature = np.hstack([enm.get_position(),
                                             enm.get_velocity()])
                    AO, TO, R = get_AO_TA_R(ego_feature, enm_feature)
                    AO = np.rad2deg(AO)
                    TO = np.rad2deg(TO)
                    shoot_interval = env.current_step - self._last_shoot_time[agent_id]

                    lock_flag = (
                            (env.agents[agent_id].is_alive or env.agents[agent_id].is_lockon) and
                            AO <= self.max_lock_angle and
                            TO <= 180 - self.max_lock_angle and
                            self.min_lock_distance <= R <= self.max_lock_distance
                    )

                    # 锁定判断
                    if lock_flag:
                        self._lock_time[agent_id] += 1
                        # print(f"{agent_id} lock enemy")
                    else:
                        self._lock_time[agent_id] = 0
                    # print(shoot_interval >= self.min_attack_interval)

                    # print(f"{agent_id}lock_time:{self._lock_time[agent_id] >= self.min_lock_step}---shoot_interval:{shoot_interval >= self.min_attack_interval}")
                    # print(self._lock_time)
                    if self._lock_time[agent_id] >= self.min_lock_step and shoot_interval >= self.min_attack_interval:
                        self._lock_time[agent_id] = 0
                        self._last_shoot_time[agent_id] = env.current_step
                        self.shoot_flag[enm.uid] = env.current_step
                        enm.lockon()
            if (env.current_step - self.shoot_flag[agent_id]) >= self.min_shoot_step:
                agent.shotdown()
...
class ManeuverAgent(BaselineAgent):
    def __init__(self, maneuver: Literal['l', 'r', 'n']) -> None:
        ...
        elif self.maneuver == 'r':
            self.target_heading_list = [np.pi / 2, np.pi, -np.pi / 2, 0]
        elif self.maneuver == 'n':
            self.target_heading_list = [np.pi, np.pi, np.pi, np.pi]
        elif self.maneuver == 'c':
            self.target_heading_list = [0, np.pi / 2, np.pi, - np.pi / 2]
        ...
        self.target_velocity_list = [180, 180, 243, 243]

    def random_maneuver(self) -> str:
        """随机选择一个机动方式"""
        # random.seed()
        return random.choice(['r', 'n', 'c'])

    def reset(self):
        self.step = 0
        self.rnn_states = np.zeros((1, 1, 128))
        self.init_heading = None
        self.maneuver = self.random_maneuver()
