...
from ..reward_functions import AltitudeReward, PostureReward, EventDrivenReward, MissilePostureReward, \
    ShootPenaltyReward, MissileShootReward
...
class MultipleCombatVSBaselineTask(SingleCombatTask):
    def __init__(self, config):
        ...
        self.die_reward_functions = [
            MissileShootReward(self.config)
        ]
...
class HierarchicalMultipleCombatVSBaselineTask(MultipleCombatVSBaselineTask):
    ...
    def __init__(self, config: str):
        ...
        self.opponent_policy = self.get_opponent_policy()
    ...
    def get_opponent_policy(self):
        args = Args()
        opponent_policy = PPOActor(args, self.observation_space, self.action_space, device=torch.device("cuda"))
        opponent_policy.eval()
        opponent_run_dir = os.path.join("D:\\", "Plane", "LAG", "scripts", "results", "MultipleCombat", "2v2",
                                   "ShootMissile", "HierarchySelfplay", "mappo", "v9", "wandb",
                                   "run-20250305_160430-xo9wxc5c", "files")
        opponent_policy.load_state_dict(torch.load(opponent_run_dir))
    ...
    def normalize_action(self, env, agent_id, action):
        ...
        if self.use_baseline and agent_id in env.enm_ids:
            ...
            enm_actions, _, enm_rnn_states = self.opponent_policy(enm_obs, enm_rnn_states, masks, deterministic=False)
            return action
        else:
...
class HierarchicalMultipleCombatShootTask(HierarchicalMultipleCombatVSBaselineTask):
    def __init__(self, config: str):
        super().__init__(config)
    ...
    def step(self, env):
        ...
        for agent_id, agent in env.agents.items():
            ...
            if not alive_enemies:
                continue  # 如果没有活着的敌机，则跳过该回合的攻击

            # 确定可以攻击最近的敌方飞机
            target_list = list(map(lambda x: x.get_position() - agent.get_position(), alive_enemies))
            target_distance = list(map(np.linalg.norm, target_list))
            target_index = np.argmin(target_distance)
            target = target_list[target_index]
            heading = agent.get_velocity()
